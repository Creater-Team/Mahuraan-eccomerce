...
const UnDeliveredOrders = () => {
  const { loading, undeliveredOrders, error } = useSelector(
    (state) => state.undeliveredOrders
  );
  ...
  useEffect(() => {
    dispatch(fetchUnDeliveredOrders());
  }, []);
  ...
  const columns = [
    ...
    {
      name: <p className="fs-6">Invoice tos</p>,
  ...
  return (
    <div className="p-3">
      <div className="flex justify-center space-x-3 items-center">
        <h1 className="text-3xl text-center font-bold">NOT DELIVERED ORDERS</h1>
        <span
        ...
        >
          {undeliveredOrders?.length && undeliveredOrders.length} orders
        </span>
      </div>{" "}
      ...
      {loading ? (
        ...
        <>
          <div className="shadow-md">
            <OrdersList
              getOrders={fetchUnDeliveredOrders}
